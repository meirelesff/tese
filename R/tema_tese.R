...
tema_tese <- function (tam.fonte = 12, fonte = "sans", rel.eixo = 0.8) {
  ...
  (ggplot2::theme_bw(base_size = tam.fonte, base_family = fonte) +
     ggplot2::theme(axis.text = ggplot2::element_text(colour = "black", size = ggplot2::rel(rel.eixo)),
                    ...
                    plot.title = ggplot2::element_text(size = ggplot2::rel(1.05), vjust = 0, margin = ggplot2::margin(0, 0, 20, 0))))
}
